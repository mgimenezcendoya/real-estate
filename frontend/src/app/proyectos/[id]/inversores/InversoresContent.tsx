...
export default function InversoresContent({ projectId }: { projectId: string }) {
  ...
  const load = async () => {
    ...
    try {
      const [inv, reps] = await Promise.all([
        api.getInvestors(projectId),
        api.getInvestorReportHistory(projectId),
      ]);
      setInvestors(inv);
      setReports(reps);
    } catch {
  ...
  const saveInvestor = async () => {
    if (!projectId || !form.nombre) return toast.error('Nombre requerido');
    setSaving(true);
    try {
      if (editingInvestor) {
        await api.patchInvestor(projectId, editingInvestor.id, form);
        toast.success('Inversor actualizado');
      } else {
        await api.createInvestor(projectId, form);
  ...
  const deleteInvestor = async (inv: Investor) => {
    ...
    try {
      await api.deleteInvestor(projectId, inv.id);
  ...
  const loadPreview = async () => {
    ...
    try {
      const p = await api.previewInvestorReport(projectId);
      setPreview(p);
    } catch (e: unknown) {
  ...
  const sendReport = async () => {
    ...
    setSending(true);
    try {
      const res = await api.sendInvestorReport(projectId, {
